...
`delegation.md`, `long-running-sessions.md`, `mcp.md`, and
`../project_documents/source-access.md` — situational extensions to
diagram 2's pipeline, read directly rather than via a diagram.

## 1. Starting a new study

From "just cloned the template" to "ready to process real PDFs." Note on
step E: `study/CLAUDE.md` loads automatically as Claude Code's memory
file when `study/` is the working directory it was started in. Starting
from the repo root works too — the root `CLAUDE.md` loads instead and
routes literature work back into `study/` — and is the better choice for
a session that will also touch project content at the root.
...
flowchart TD
    ...
    C --> D["User drops ../project_documents/<br/>brief, proposal, provided material"]
...
flowchart TD
    ...
    H --> RAT["Coordinator ratifies the collection-derived fields<br/>tags, scope verdict, gap flags"]
    RAT --> I{"Head-to-head comparison<br/>or ranked survey?"}
    ...
    J -- no --> K
    J1 --> K["Run: build_reference_index.py<br/>then build_catalog_index.py"]
...
flowchart TD
    ...
    T -- "What are my options<br/>for X?" --> B1["Read catalog-index.md in full,<br/>filter by topics/vocabulary.md tags"]
    ...
    subgraph COST["Read-cost discipline, cheapest first"]
        direction LR
        X0["catalog-index.md<br/>read in full, always"] --> X1["catalog.md<br/>grep one entry block, never full-read"]
        X1 --> X2["topics/*.md dossier<br/>pre-synthesized, cheap"]
